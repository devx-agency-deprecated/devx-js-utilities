 ...
 */
export const salutation = (name: string) => {
  let convertName
  let replaceName
  let character
  ...
  if (replaceName[0] == '') {
    return (
        name +
        (jmeno.charAt(jmeno.length).toLowerCase() == name.charAt(name.length)
            ? replaceName[1]
            : replaceName[1].toUpperCase())
    )
  }
  let replaceending
  ...
  if (replaceending.match(/^[A-ZÁČĎÉÍŇÓŘŠŤÚÝŽ][a-záčďéěíňóřšťúůýž]*$/u)) {
    return (
        name.substr(0, name.length - replaceName[0].length) +
        replaceName[1].replace(/\w\S*/g, function(txt) {
